...
import dsbd2020.project.productmanager.support.TopicMessage;
...
public class KafkaOrder {
    ...
    public void listenProductTopic(String message) {
        ...
        TopicMessage order_request;
        if (message != null) {
            order_request = new Gson().fromJson(message, TopicMessage.class);
            Map<Integer, Integer> extraArgs = new HashMap<>();
            if (order_request.getKey() == "order_completed") {
                updateRequest = (ProductUpdateRequest) order_request.getValue();
            ...
            TopicMessage orderValidation = new TopicMessage()
